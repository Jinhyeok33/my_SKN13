 /* 부모, 자식 테이블 설명
 직원들 정보가 적힌 table, 각 부서별 정보가 적힌 table이 있다.
 각 직원들은 부서를 하나씩 가지고, 그 부서는 부서용 table에 해당하는 정보를 참조한다.
 따라서 직원 table이 자식 테이블, 부서 table이 부모 테이블이 되며
	이에 따라 자식 테이블의 각 데이터는 부모 테이블의 Primary Key(PK) 값을 참조하게 된다.
	참조한다는 것은 하나의 별도 칼럼을 통해 부모 테이블의 PK 값을 저장해놓는다는 것.
		-> 이 Key를 Foreign Key 라고 함
 부모 테이블을 자식 데이터의 추가 데이터, 추가 정보라고 생각하면 되겠다.
 자식 테이블에서 FKey 를 통해 올라가야 부모 테이블로 접근하는거니까... 자식 부모가 이게 맞는듯
 -> ERD(Entity-Relation-Diagram)이 이 관계를 나타내주는 그림
 
 이런 테이블 간의 구조가 왜 필요하느냐?
 하나의 테이블에 각 데이터들에 대한 모든 정보들(칼럼들) 을 적어놓기에는 데이터들이 공유하는 특징
	(같은 부서를 다닌다든가)이 있고, 비슷한 특징을 공유하는 칼럼(직원 테이블에 '부서명' 칼럼,
	'부서위치' 칼럼, ...)이 존재하기 때문에 테이블 간의 연결성이 생길 수 밖에 없다.
 */
 /*CONSTRAINT 구문
 자식 테이블에 참조되어 있는 부모 테이블의 값은 함부로 삭제/수정할 수 없다.(다른 곳에서 쓰니까)
 만약, 부모 테이블을 삭제하려면 Foreign Key를 지운 뒤 삭제하고 다시 설정해주면 된다.
	-> ppt
 이들을 연결시켜주는 일은 CONSTRAINT 구문을 통해 할 수 있다. 이때 필요한 값은 자식 테이블의 어떤
	칼럼에 부모 FK값을 넣을건데? 와 어떤 부모 테이블의 어떤 칼럼 값을 넣을건데? 이다.
 그래서 ~ CONSTRAINT '연결'자체의 이름 FOREIGN KEY(자식 칼럼) \
	REFERENCES부모테이블(부모칼럼) [on 설정] 의 형식을 따른다.
 연결 자체의 이름은 자유이며 관례적으로 fk_자식테이블_부모테이블 의 형식을 따른다.
 on 설정의 경우, 연결된 부모 칼럼 값이 삭제/수정되면 그걸 참조한 자식 칼럼 값은 어떻게 할거야..?
	에 관한 설정이며 on DELETE / on UPDATE 구문을 쓴다.
 설정방식은 두 가지로 뒤에 CASCADE를 적으면 삭제/수정된 부모 칼럼 값을 그대로 계승한다는 의미이며,
	뒤에 SET NULL을 적으면 부모 칼럼 값이 삭제/수정됐을 때 자식 칼럼값을 그냥 null로 한다는
	의미를 가지고 있다.
 당연하게도 이 CONSTRAINT 구문은 참조를 하는 자식 테이블 설정 시에만 규정해주어야한다.
 */
