...
# 이런 입력을 'event 처리' 해야하며 이들을 java_script가 해준다. / event란 값이 새로 생기거나 바뀌는 것
# 이를 streamlit은 이벤트 발생 시 처음부터 모든 코드를 재시작함으로서 java_script를 대신한다.
# 이러니 java_script가 흥행하지.... ////// 캐시를 이용하면 된댜
#########################################

# 페이지 설정
st.set_page_config(page_title="✏️Input Widget✏️", layout="wide") # -> 페이지 위에 나오는 간략 제목
...
####### 다중 파일 업로드.
# st의 파일은 여러 속성(.name, 길이, 용량 등등) 을 가지고 있다.
